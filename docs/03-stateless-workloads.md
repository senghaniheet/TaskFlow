# 03 ó Stateless Workloads: Pods & Deployments

> **Prerequisites:** [02 ó Namespaces: Virtual Clusters & Resource Governance](./02-namespaces.md)

---

## ≠É∫· Theory: The Workload Hierarchy
...
![Kubernetes Architecture ‘«ˆ All Objects](../assets/kubernetes-architecture.jpg)

**How to read this diagram:**

- **Top-left legend:** A reference card for every K8s object type. Use it as a cheat sheet ‘«ˆ each shape/colour maps to a specific resource kind.
- **User ‘Â∆ Internet ‘Â∆ Ingress:** All external traffic enters through the Nginx Ingress Controller, which routes to the correct Service based on path or host.
- **Inside the Namespace (`my-app`):** This is where your application lives. Notice it's sandboxed ‘«ˆ resources in other namespaces can't conflict with these names.
- **Service ‘Â∆ Deployment ‘Â∆ Pods:** The API (stateless) side. The Service has a stable ClusterIP. The Deployment manages 3 identical, interchangeable Pods.
- **Service ‘Â∆ StatefulSet ‘Â∆ Pods:** The Database (stateful) side. Each pod (`mydb-0`, `mydb-1`, `mydb-2`) has its own identity and its own PVC ‘Â∆ PV chain.
...
> **The key pattern:** Every connection is managed by a Kubernetes **controller** watching for state changes. Labels + selectors do the binding ‘«ˆ nothing is wired up manually.

---

## Pod ‘«ˆ The Atomic Unit
...
```yaml
# 01-pod.yaml ‘«ˆ for learning only; use a Deployment in production
apiVersion: v1
kind: Pod
metadata:
...
spec:
  containers:
    - name: api
      ...
      ports:
        - containerPort: 5000   # Documentation only ‘«ˆ does not open the port

      env:
        - name: NODE_ENV
          value: "production"
        - name: JWT_SECRET
          value: "replace-me"   # Never hardcode real secrets ‘«ˆ use a Secret resource
...
### ‘Â∆ Try It: Apply and Observe a Pod
...
# Should go: Pending ‘Â∆ Running

# Inspect it
kubectl describe pod taskflow-api-pod -n taskflow
# Read the Events section at the bottom ‘«ˆ shows every step K8s took
...
# Now simulate a crash ‘«ˆ delete the pod
...
# ‘ÂÊ This is the problem. No one recreated it. Use a Deployment instead.
```

> **What you just proved:** A naked Pod is NOT self-healing. When it's deleted ‘«ˆ by you, a crash, or a node failure ‘«ˆ it stays dead. This is exactly why Deployments exist.

---

## Deployment ‘«ˆ Managing Stateless Replicas
...
- ‘£‡ **Scaling:** `replicas: 3` ‘Â∆ `replicas: 10` instantly
...
Initial state (3 pods running, all old version):
  [api-abc] [api-def] [api-ghi]   ‘Â… old pods

Step 1: Create 1 new pod (4 pods total):
  [api-abc] [api-def] [api-ghi]   ‘Â… old
  [api-xyz]                        ‘Â… new (starting...)

Step 2: New pod passes readiness probe:
  [api-abc] [api-def] [api-ghi]   ‘Â… old (serving)
  [api-xyz]                        ‘Â… new (serving)

Step 3: Kill 1 old pod (back to 3):
  [api-def] [api-ghi]             ‘Â… old
  [api-xyz]                        ‘Â… new

  ... repeat until all replaced ...

Final state:
  [api-xyz] [api-uvw] [api-rst]   ‘Â… all new
```

**The readiness probe is the gatekeeper.** If the new pod fails the readiness probe, the rollout pauses ‘«ˆ old pods keep serving. No downtime.
  ...
  template:
    metadata:
      labels:
        app: api
      annotations:
        # sha256 of ConfigMap/Secret content ‘«ˆ changes here trigger a rolling restart
...
### ‘Â∆ Try It: Apply and Observe a Deployment
...
# Watch pods come up ‘«ˆ notice random hash names (not taskflow-api-0, 1, 2)
...
> **What you just proved:** Deployments self-heal, scale, and roll out ‘«ˆ all without downtime. But notice the problem: all config is hardcoded in the YAML. To run this in staging with 1 replica, you'd need a second copy of the file. We'll solve this in [Chapter 05 ‘«ˆ Helm](./05-helm.md).
...
> [!NOTE]
> StatefulSets (for databases like MongoDB) are covered in [07 ó StatefulSets](./07-statefulsets.md), after you've learned about Persistent Volumes in [06 ó Storage](./06-storage.md).

---

**Next:** [04 ó Networking: Services, Ingress, and DNS ?](./04-networking.md)
